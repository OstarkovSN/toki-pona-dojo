...
export function useChat(options: UseChatOptions): UseChatReturn {
  ...
  const buildRouteHint = useCallback((): string | undefined => {
    ...
    switch (page) {
      ...
      case "dictionary":
        return detail ? `The learner is looking at the word '${detail}'` : undefined;
      case "grammar":
    ...
    async (content: string) => {
      ...
      try {
        if (byomConfig) {
          ...
          await callProviderDirect(byomConfig, fullMessages, onChunk, controller.signal);
        } else {
          // Path 1: Server proxy
          await callServerProxy(messagesForApi, chatCtx, onChunk, controller.signal);
        }
      } catch (err: unknown) {
