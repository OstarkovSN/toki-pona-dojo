...
- **File-level `test.use({ storageState: ... })` overrides ALL projects** — applying `storageState` override at file level breaks authenticated projects (`[chromium]`, `[mobile-chrome]`); removal is critical for multi-project suites.
- **`page.addInitScript()` runs before page load** — suitable for injecting localStorage tokens (e.g., `localStorage.setItem("access_token", "fake-test-token")`); for authenticated projects it's a no-op (token already present), for `[no-auth]` it enables auth guards to pass.
- **Pre-existing Biome `useSemanticElements` lint errors** on 6 skeleton components (`ChatTypingIndicator.tsx`, `DictionarySkeleton.tsx`, `GradingSpinner.tsx`, `GrammarSkeleton.tsx`, `LessonSkeleton.tsx`, `SkillTreeSkeleton.tsx`) are infrastructure issues unrelated to test fixes; these components use `role="status"` which Biome prefers as `<output>`, requiring manual element changes.
- **`getByRole("link", { name: "dictionary" })` finds plain `<Link>` elements by text label** — TopNav renders text labels without aria-label overrides; this selector works reliably.
- **`page.route()` for mocking API calls uses glob patterns** — pattern `**/api/v1/dictionary/words` (no trailing path) matches the exact endpoint; `localhost:5173` silently returns HTML for unmocked `/api/*` unless `server.proxy` is configured in `vite.config.ts`.
- **`_layout.tsx` `beforeLoad` guard checks `isLoggedIn()`** — all authenticated pages require a real or mocked `localStorage` access token; tests without tokens silently redirect via `window.location.href = "/login"`.
- **`handleApiError` in `main.tsx` uses `window.location.href` for 401/403** — this bypasses TanStack Router's `navigate()` and appears as a hard navigation; only `page.route()` mocks can prevent the redirect.
- **Radix UI DropdownMenu closes after item click; reopening requires `waitFor({ state: "hidden" })`** on `[role="menu"]` before the next click; without it, the second click fires during animation and the open event is swallowed.
- **ChatPanel Sheet overlay (`inset-0` backdrop) on mobile blocks pointer events to elements outside it** — ChatPanel must render outside `SidebarInset` as a peer, not a child; full-viewport overlay coverage prevents clicks to the header even though it's visually outside the 80vh bottom sheet.
