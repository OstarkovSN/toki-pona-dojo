...
- **TopNav prop-based pattern replaced with context hook** — `TopNav` on master took `onToggleChat`/`chatOpen` props but was never rendered in `_layout.tsx`; PR #22 integrates it AND switches from props to `useChatContext()`. Both refactoring+integration happened in same commit; splitting them would clarify intent.
